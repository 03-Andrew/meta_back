...
class ExcelMetadataHelper:
    """
    Helper class for extracting and deleting metadata from .xlsx files using ZIP + XML parsing.
    """
    _parts = {
    ...
    def get_metadata(file_path: str) -> dict[str, str]:
        ...
        with zipfile.ZipFile(file_path) as xlsx:
            for part, path in ExcelMetadataHelper._parts.items():
                try:
                    xml_bytes = xlsx.read(path)
                except KeyError:
                    continue
                ...
                if part in ('core', 'app'):
                    ...
                    for prop in root.findall(f'.//{{{ExcelMetadataHelper._ns_custom}}}property'):
                        name = prop.get('name')
                        value = ''
                        for child in prop.findall(f'./{{{ExcelMetadataHelper._ns_vt}}}*'):
                            value = child.text or ''
                            break
    ...
    def delete_metadata(file_path: str, fields_to_delete: list[str]) -> None:
        ...
        with zipfile.ZipFile(file_path, 'r') as zin, \
            ...
            for item in zin.infolist():
                ...
                if item.filename in ExcelMetadataHelper._parts.values():
                    part_name = next(
                        key for key, val in ExcelMetadataHelper._parts.items()
